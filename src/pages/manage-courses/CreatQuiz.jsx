...
const CreateQuiz = () => {
  ...
  const { courseId } = useAuth(); // Fetch course ID from auth context
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_HOST}/courses/${courseId}`)
      .then((response) => {
  ...
  const handleSubmit = (e) => {
    ...
    Promise.all(
      quizzes.map((quiz) => {
        ...
        return axios.post(
          `${import.meta.env.VITE_HOST}/courses/${courseId}/chapters/${
            quiz.chapterId
          }/quiz`,
          quizData
        );
